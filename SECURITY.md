...
[advisory]: https://github.com/savindaJ/SLSKIT-CLI/security/advisories/new
...
- **Profile resolution on deploy** is `--profile`, then the name stored in
  `slskit.json`, then `AWS_PROFILE`. A leftover shell profile cannot override the
  one this project recorded. Access keys still never enter the repo.
