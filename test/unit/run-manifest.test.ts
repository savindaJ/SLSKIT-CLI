...
describe("readManifest", () => {
  it("throws when sless.json is missing", () => {
    const dir = createTempDir("sless-run-manifest-missing-");
  ...
  it("throws when the project framework is not sam", () => {
    const dir = createTempDir("sless-run-manifest-framework-");
    fs.writeFileSync(
      path.join(dir, "sless.json"),
      JSON.stringify({ name: "demo", framework: { id: "serverless" } })
    );

    expect(() => readManifest(dir)).toThrow(/supports SAM projects only/);
    removeDir(dir);
  });

  it("returns the manifest for a sam project", () => {
    const dir = createTempDir("sless-run-manifest-ok-");
    fs.writeFileSync(
      path.join(dir, "sless.json"),
